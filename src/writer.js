...
// let output = yaml.safeDump(post.frontmatter);
...
async function loadMarkdownFilePromise(post) {
  ...
  try {
    const frontmatterYaml = yaml.dump(post.frontmatter, { skipInvalid: true });
    output = `---\n${frontmatterYaml}---\n\n${post.content}\n`;
  } catch (error) {
    console.error(`Error parsing frontmatter YAML for ${post.filename}: ${error.message}`);
    output = `${post.content}\n`;
  }
...
function getPostPath(post, config) {
  ...
  pathSegments.push(slugFragment, 'default.md');
