...
function getPostSlug(post) {
  postIndex++;
  return decodeURIComponent(leadingZero(postIndex, 2) + "." + post.post_name[0]);
}
...
function collectAttachedImages(data) {
  const images = getItemsOfType(data, 'attachment')
    ...
    .map(attachment => ({
      ...
      url: attachment.attachment_url[0].replace(/-\d+x\d+(\.\w+)$/, '$1')
    }));
...
function collectScrapedImages(data, postTypes) {
  ...
  postTypes.forEach(postType => {
    getItemsOfType(data, postType).forEach(post => {
      ...
      const matches = [...postContent.matchAll(/<img[^>]*src="(.+?\.(?:gif|jpe?g|png))(?:-\d+x\d+)?(\.\w+)"[^>]*>/gi)];
      matches.forEach(match => {
        // base the matched image URL relative to the post URL, removing the size portion of the filename if present
        const url = new URL(match[1] + match[2], postLink).href;
        images.push({
          id: -1,
          postId: postId,
          url
